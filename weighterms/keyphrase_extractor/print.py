...
def read_and_dump(
        doc):  # create a master corpus from all the articles , and use these corpus for probability calculation
    return doc['content']
...
def extract_keyphrase(data, unigram_count, bigram_count):  # Function to extract keyphrases from the given corpus
    ...
    for item in data:  # data is a list of dictinory
        ...
        article_content = str(unicode(item['content']))
        ...
        bigram = list(collections.Counter(
            izip(words, islice(words, 1, None))))  # bigram contains list of bigrams that we need to check
        for i, j in bigram:  # i, is the first word and j is the second word
            if bigram_count[(i, j)] != unigram_count[i]:
                ...
                prob = float(-2 * compute_likelihood(unigram_count[i], unigram_count[j], bigram_count[(i, j)],
                                                     len(bigram_count)))
...
def calculate(k, n, x):
    ...
    try:
        if k > 0 and n > k and k is not None and n is not None:
            first_tearm = x ** k
            second_term = (1 - x) ** (n - k)
...
jobs = [(job_server.submit(read_and_dump, (input,))) for input in
        data]  # Parallel computation to collect the corpus from different articles
...
final_job = job_server.submit(extract_keyphrase, (data, unigram_count, bigram_count,),
                              (calculate, compute_likelihood), )
result = final_job()
print result
