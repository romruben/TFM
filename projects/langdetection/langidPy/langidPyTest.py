...
def testLangidPy(testdir, languages, filesByLang):
    ...
    #files to filter
    filesToFilter = [file for file in testFilesReference.keys() if testFilesReference.get(file) not in languagesSupported]
    ...
    #well detected files
    detected = sum(1 for file in filesToTest if languagesIds.get(langid.classify(open(TEST_FILES_DIR+file, 'r').read())[0]) == testFilesReference.get(file))

    #testing
    if(len(filesToTest) != detected): print "Expected %r, Detected %r" % (len(filesToTest), detected)
