...
//HiddenMarkovModel test, expected: 2020000 obtained: 2020000 in PT-53M-7.401S
...
public class LingPipeTestsWithADoc {
    ...
    private static void evaluateLingPipe() {
        List<String> files = Stream.of((new File("/var/tmp/docs/txt/")).listFiles()).map(f -> f.getAbsolutePath()).collect(toList());
